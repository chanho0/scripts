...
const inviteCodes = ['T015vf11RBka9VLWT0cCjVWnYaS5kRrbA&T0225KkcRx4YplTeJh-hxaVedACjVWnYaS5kRrbA&T0225KkcRR4do1yEcR6mx_AMJgCjVWnYaS5kRrbA&T0205KkcI11siCCyVFOyypF9CjVWnYaS5kRrbA&T018v_5zQhsc8FTWIR6b1ACjVWnYaS5kRrbA'];
!(async () => {
  await requireConfig();
  if (!cookiesArr[0]) {
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `https:///api/ddfactory/${randomCount}`, headers:{'Host':'api.jdsharecode.xyz'}, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
