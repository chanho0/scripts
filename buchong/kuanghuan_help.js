  ...
  for (let m = 0; m < cookiesArr.length; m++) {
    ...
    if (cookiesArr && cookiesArr.length >= 2) {
      ...
      for (let j = 0; j < $.shareId .length && $.canHelp; j++) {
        console.log(`账号 ${$.index} ${$.UserName} 开始给 ${$.shareId [j]} 进行助力`)
        $.max = false;
        try {

          await dohelp($.shareId [j])
...
function gethelp() {
  ...
  const body = {"apiMapping":"/api/task/support/getShareId"};
...
function dohelp(shareId) {
  //一个人一天只能助力两次，助力码inviteCode  每天都变
  const body = {shareId,"apiMapping":"/api/task/support/doSupport"};
  const options = taskUrl("collect_bliss_cards_prod", body)
  return new Promise((resolve) => {
    $.post(options, (err, resp, data) => {
      try {
        if (err) {
          ...
          if (data) {
            if (data.code===200) {
              if(data.data && data.data.status===7){
                console.log(`\n助力成功`)
              }else if(data.data && data.data.status===1){
                console.log(`\n不能给自己助力`)
              }else if(data.data && data.data.status===5){
                console.log(`\n助力次数已用完`)
                $.canHelp = false;
              }else{
                console.log(`\n助力失败`)
...
function taskUrl2(functionId, body = {}) {
  return {
    url: `${JD_API_HOST}?appid=china-joy&functionId=${functionId}&client=wh5&clientVersion=1.0.0&appid=o2_act&uuid=${$.uuid}`,
    body: `body=${escape(JSON.stringify(body))}`,
    headers: {
      "Host": "api.m.jd.com",
      "Content-Type": "application/x-www-form-urlencoded",
