  ...
  for (let m = 0; m < cookiesArr.length; m++) {
    ...
    if (cookiesArr && cookiesArr.length >= 2) {
      ...
      for (let j = 0; j < $.groupId .length && $.canHelp; j++) {
        console.log(`账号 ${$.index} ${$.UserName} 开始给 ${$.groupId [j]} 进行助力`)
        $.max = false;
        await jinli_h5assist($.groupId [j]);
        await $.wait(2000)
	if ($.max) {
          $.groupId .splice(j, 1)
          j--
...
function red() {
  const body = {"activityKey":"53275a405ec3cba14b28662e05f6c53b"};
  const options = taskUrl("necklacecard_openGroup", body)
      ...
      try {
        if (err) {
          ...
          if (data && data.data ) {
            ...
            console.log(`助力结果：${data.data.biz_msg}`)
            if (data.data.biz_code === 7) $.max = true;
...
function open() {
  //一个人一天只能助力两次，助力码groupId  每天都变
  const body = {"activityKey":"53275a405ec3cba14b28662e05f6c53b"};
  const options = taskUrl("necklacecard_openCard", body)
  return new Promise((resolve) => {
    $.post(options, (err, resp, data) => {
      try {
        if (err) {
...
function taskUrl(functionId, body = {}) {
  return {
    url: `${JD_API_HOST}?appid=coupon-necklace&client=wh5&functionId=${functionId}&t=${new Date().getTime() * 1000}`,
