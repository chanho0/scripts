...
function Info() {
  ...
  const body = {"apiMapping":"/api/index/indexInfo"};
...
function Open() {
  //一个人一天只能助力两次，助力码inviteCode  每天都变
  const body = {"apiMapping":"/api/lottery/lottery"};
  const options = taskUrl("collect_bliss_cards_prod", body)
  return new Promise((resolve) => {
    $.post(options, (err, resp, data) => {
      try {
        if (err) {
          ...
          if (data  ) {
            if (data.code===200 &&data.data) {
                ...
                console.log(`\n\n抽到：${data.data.prizeName} ${data.data.prizeCount}`)
