...
async function Gethelp() {
  try {
    ...
    await takeGetRequest('Gethelp');
      ...
      console.log(`互助码：${$.helpInfo.myInfo.shakeKey}`);
      $.shakeKey=$.helpInfo.myInfo.shakeKey
...
async function takeGetRequest(type) {
  ...
  switch (type) {
    ...
    case 'Gethelp':
      url = `https://m.jingxi.com/jxnhj/GetUserInfo?__t=${Date.now() + 2}&dwEnv=7&strInviteId=&nopopup=1&_stk=__t%2CdwEnv%2Cnopopup%2CstrInviteId&_ste=1`;
      ...
      break;
    case 'Dohelp':
      url = `https://m.jingxi.com/jxnhj/Draw?__t=${Date.now() + 2}&dwEnv=7&strSceneType=${$.shakeKey}&scene=11&bizch=&_stk=__t%2Cbizch%2CdwEnv%2Cscene%2CstrSceneType&&_ste=1`;
      url += `&h5st=${decrypt(Date.now(), '', '', url)}&&sceneval=2&g_login_type=1&callback=jsonpCBK${String.fromCharCode(Math.floor(Math.random() * 26) + "A".charCodeAt(0))}&g_ty=ls`;
      break;
  ...
  switch (type) {
    case 'GetHomePageInfo':
      ...
      if (data.iRet === 0) {
        $.retInfo = data;
        console.log(`获取活动信息异常：${JSON.stringify(data)}\n`);
        console.log(`助力：${$.retInfo.sErrMsg}获得${$.retInfo.data.discount}${$.retInfo.data.prizeDesc}${$.retInfo.data.prizeName}\n`);
