...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            $.activityId = 'dz846b4029450592f6df4483e8b369'
            $.activityShopId = '1000013402'
            $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/yili/rhythmmaster/activity/${$.authorNum}?activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}&adsource=null&shareuserid4minipg=${encodeURIComponent($.secretPin)}&shopid=${$.activityShopId}&lng=00.000000&lat=00.000000&sid=&un_area=`
...
async function member() {
    ...
    if ($.token) {
        ...
        if ($.secretPin) {
            ...
            if ($.index === 1) {
                await task('yili/rhythmmaster/activityContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=${encodeURIComponent($.authorCode)}&loversUuid=`, 0, 1)
            } else {
                await task('yili/rhythmmaster/activityContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=${encodeURIComponent($.authorCode)}&loversUuid=`)
            }
            await task('taskact/common/drawContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}`)
            await task('yili/rhythmmaster/initOpenCard', `pin=${encodeURIComponent($.secretPin)}&activityId=${$.activityId}`)
            lnzl++;
            // $.log("加入店铺会员")
            // if ($.openCardList) {
            //     $.log(">>> 模拟上报访问记录")
            //     for (let i = 0; i < ($.openCardList.length); i++) {
            //         await task('crm/pageVisit/insertCrmPageVisit', `venderId=1000013402&pageId=dz846b4029450592f6df4483e8b369&elementId=入会跳转&pin=${encodeURIComponent($.secretPin)}`, 1)
            //         await $.wait(2000)
            //     }
            ...
            console.log('任务 -> ')
            await task('yili/rhythmmaster/saveTask', `activityId=${$.activityId}&actorUuid=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&taskType=23`)
            await $.wait(2000)
            await getFirstLZCK()
            await getToken();
            await task('yili/rhythmmaster/saveTask', `activityId=${$.activityId}&actorUuid=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&taskType=21`)
            await $.wait(2000)
            await getFirstLZCK()
            await getToken();
            await task('yili/rhythmmaster/saveTask', `activityId=${$.activityId}&actorUuid=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&taskType=90`)
            await $.wait(2000)
            await getFirstLZCK()
            await getToken();
            await task('yili/rhythmmaster/saveTask', `activityId=${$.activityId}&actorUuid=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&taskType=12`)
            await $.wait(2000)
            await getFirstLZCK()
            await getToken();
            await task('yili/rhythmmaster/saveTask', `activityId=${$.activityId}&actorUuid=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&taskType=92`)
            await $.wait(2000)
            ...
            for (let j = 0; j < 15; j++) {
                console.log("开始游戏");
                await task('yili/rhythmmaster/startGame', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}`)
                await $.wait(31000)
                console.log("结束游戏");
                await task('yili/rhythmmaster/endGame', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&gameId=${$.gameId}&score=100`)

                if (lngameTimes == 0)
                    break;
                else
                    console.log("游戏次数剩余: " + lngameTimes);
                        ...
                        if (data.result) {
                            switch (function_id) {
                                ...
                                case 'wxActionCommon/getUserInfo':
                                    break;
                                case 'yili/rhythmmaster/activityContent':
                                    if (!data.data.hasEnd) {
                                        $.log(`开启【${data.data.activityName}】活动`)
                                        $.log("-------------------")
										console.log("你的助力码 -> "+data.data.actorUuid);
                                        if ($.index === 1) {
                                            ownCode = data.data.actorUuid
                                        } else {
											if(data.data.actorUuid){
                                    ...
                                    break;
                                case 'yili/rhythmmaster/initOpenCard':
                                    $.openCardList = data.data.openInfo;
                                    $.openCardStatus = data.data;
                                    // console.log(data)
                                    break;
                                case 'yili/rhythmmaster/startGame':									
                                    console.log(data)
                                    $.gameId = data.data.gameId;
									lngameTimes=data.data.assistCount;
                                    break;
                                case 'yili/rhythmmaster/endGame':
                                    console.log(data)
                                    break;
                                ...
                                case 'linkgame/assist':
                                    $.log(JSON.stringify(data))
                                    break;
                                case 'yili/rhythmmaster/saveTask':
                                    console.log(data)
                                    break;
                                default:
                                    // $.log(JSON.stringify(data))
