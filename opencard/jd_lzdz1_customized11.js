...
!(async () => {
  ...
  authorCodeList = await getAuthorCodeList('https://raw.githubusercontent.com/okyyds/duck/master/code/jsakf.json')
  console.log(authorCodeList)
  if ($.getAuthorCodeListerr === false) {
      authorCodeList = [
        '519e62d11bb247d08ccf5f27af10bd33'
      ]
  }
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      // authorCodeList = [
      //   // '141c4499d91f4b5eac74ef27a49d749c',
      //   '6d823fe7d9b34d7fb3bb605bba9cd978',
      //   // 'f3b9220223564824996e1f4ad967d197',
      // ];
      ...
      $.activityShopId = "1000001683";
      $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/baby/pairysactivity?activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}&adsource=undefined/activity/8258555?activityId=dzf14101834564a0b1336646135829&shareuserid4minipg=${encodeURIComponent($.secretPin)}&shopid=1000001683&sid=&un_area=`;
      await member();
      //await $.wait(500);
      if ($.bean > 0) {
    ...
    if ($.secretPin) {
      ...
      if ($.openCardList) {
        for (const vo of $.openCardList) {
          console.log(vo.openCard)
          $.log(`>>> 去加入 ${vo.venderId}`);
          if (vo.openCard === false) {
            $.log(`>>> 准备加入会员`);
      ...
      // await task('baby/pairysactivity/startGame', `pin=${encodeURIComponent($.secretPin)}&activityId=${$.activityId}`)
      // await task('baby/pairysactivity/endGame', `pin=${encodeURIComponent($.secretPin)}&activityId=${$.activityId}&gameId=${$.gameId}&score=${$.score}`)
      // console.log('任务 -> ')
      // //await $.wait(2000)
      ...
      // console.log('抽奖 -> ')
      // //await $.wait(2000)
      // await task('baby/pairysactivity/draw', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}`);
      // //await $.wait(2000)
      // await task('baby/pairysactivity/draw', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}`);
      // //await $.wait(2000)
      // await task('baby/pairysactivity/draw', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}`);
            ...
            if (data.result) {
              switch (function_id) {
                ...
                case "wxActionCommon/getUserInfo":
                  break;
                case "baby/pairys/activityContent":
                  if (!data.data.hasEnd) {
                  ...
                  break;
                case "baby/pairys/initOpenCard":
                  $.openCardList = data.data.openInfo;
                  // $.openCardStatus = data.data.allOpenCard;
                  // console.log(data)
                  break;
                case "baby/pairys/followShop":
                  console.log(data);
                  break;
                case "baby/pairysactivity/draw":
                  console.log(data);
                  break;
