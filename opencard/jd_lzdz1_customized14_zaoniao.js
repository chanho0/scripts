...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      authorCodeList = [
        '709b9a791c6143a589b91a3903b80e28'
      ];
      ...
      $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/april/springUnion2/activity/${$.authorNum}?activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}&adsource=null&shareuserid4minipg=null&shopid=undefined&lng=00.000000&lat=00.000000&sid=&un_area=`;
      await member();
      //await $.wait(1000);
      if ($.bean > 0) {
...
async function member() {
  ...
  if ($.token) {
    ...
    if ($.secretPin) {
      ...
      if ($.index === 1) {
        await task("april/springUnion2/activityContent", `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=${encodeURIComponent($.authorCode)}`, 0, 1);
      } else {
        await task("april/springUnion2/activityContent", `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=${encodeURIComponent($.authorCode)}`);
      }
      $.log("关注店铺");
      await task("april/springUnion2/saveTask", `activityId=${$.activityId}&actorUuid=${encodeURIComponent($.actorUuid)}&pin=${encodeURIComponent($.secretPin)}&taskType=23&taskValue=23&shareUuid=${encodeURIComponent($.authorCode)}`);
      //await $.wait(1000);
      ...
      await task('april/springUnion2/draw', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}`);
    }
  }
}
            ...
            if (data.result) {
              switch (function_id) {
                ...
                case "wxActionCommon/getUserInfo":
                  break;
                case "april/springUnion2/activityContent":
                  if (!data.data.hasEnd) {
                  ...
                  break;
                case "april/springUnion2/initOpenCard":
                  $.openCardList = data.data.openInfo;
                  $.openCardStatus = data.data.allOpenCard;
                  // console.log(data)
                  break;
                case "april/springUnion2/saveTask":
                  console.log(data);
                  break;
                case "linkgame/sign":
                  console.log(data);
