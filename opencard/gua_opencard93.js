...
All变量适用
————————————————
入口：[ 1.6～1.31 会员狂欢日 (https://lzdz1-isv.isvjcloud.com/dingzhi/bigname/memberday/activity?activityId=dz2112100022468201&shareUuid=ca650078f09d43be8d535f36712ab9f2)]
...
process.env.guaopencard93 = true
...
!(async () => {
  ...
  $.shareUuid = "b310755739474baea1acf02ff29f858a"
...
async function run() {
  try {
    ...
    if($.allOpenCard == false){
      ...
      for(o of $.openList){
        ...
        if(o.openStatus == false){
          ...
          let join2Resp = await joinShop();
          let join2RespTryCount = 0;
          while (join2Resp.indexOf('火爆') !== -1 && join2RespTryCount <= 10) {
            $.log('===== 重试入会 ' + (join2RespTryCount + 1))
            join2RespTryCount ++
            await $.wait(100)
            join2Resp = await joinShop();
          }
          await takePostRequest('activityContent');
          await takePostRequest('drawContent');
          await takePostRequest('checkOpenCard');
    ...
    if(guaopenwait){
      ...
      if($.index % 20 == 0) console.log('休息1分钟，别被黑ip了\n可持续发展')
      if($.index % 20 == 0) await $.wait(parseInt(Math.random() * 5000 + 10000, 10))
    }
  } catch (e) {
    console.log(e)
  }
  ...
  switch (type) {
    case 'isvObfuscator':
      ...
      case 'getShareRecord':
        ...
        break;
      case 'getRule':
        url = `${domain}/dingzhi/bigname/memberday/getRule`;
        body = `activityId=${$.activityId}`
        break;
      case '抽奖':
        url = `${domain}/dingzhi/bigname/memberday/draw`;
        body = `activityId=${$.activityId}&pin=${encodeURIComponent($.Pin)}`
        break;
      default:
        console.log(`错误${type}`);
