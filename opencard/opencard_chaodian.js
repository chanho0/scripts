...
async function superFans() {
    ...
    if ($.token) {
        ...
        if ($.secretPin) {
            ...
            await task('shop/league/activityContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=${encodeURIComponent($.pinImg)}&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=${encodeURIComponent($.authorCode)}`)
            if ($.activityContent) {
                ...
                if ($.openCardStatus) {
                    ...
                    for (const vo of taskList) {
                        $.log(`去加入${vo.name}`);
                        if (vo.value === vo.value2) {
                            let bindWithVenderResp = await bindWithVender(
                                {
                                    venderId: `${vo.value}`,
                            ...
                            while (bindWithVenderResp.indexOf('火爆') !== -1 && join2RespTryCount <= 10) {
                                $.log('===== 重试入会 ' + (join2RespTryCount + 1))
                                join2RespTryCount ++
                                await $.wait(100)
                                bindWithVenderResp = await bindWithVender(
                                    {
                                        venderId: `${vo.value}`,
                                        bindByVerifyCodeFlag: 1,
                                        registerExtend: {},
