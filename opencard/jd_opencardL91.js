...
const $ = new Env('3.2-3.8 诠释你的每一面');
const jdCookieNode = $.isNode() ? require('./jdCookie_502457532.js') : '';
...
!(async () => {
    ...
    if(authorCodeList === '404: Not Found'){
        authorCodeList = [
            'fce501f89ac34bc8bc69d06b77a97328',
        ]
    }
...
async function run() {
    try {
        ...
        if($.index % 3 == 0) await $.wait(parseInt(Math.random() * 5000 + 11111, 10))
    } catch (e) {
        console.log(e)
    }
}
...
async function takePostRequest(type) {
    ...
    switch (type) {
        ...
        case 'getShareRecord':
            ...
            break;
        case 'getRule':
            url = `${domain}/dingzhi/fashion/union/getRule`;
            body = `activityId=${$.activityId}`
            break;
        case '抽奖':
