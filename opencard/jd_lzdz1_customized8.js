...
async function member() {
    ...
    if ($.token) {
        ...
        if ($.secretPin) {
            ...
            if ($.opencard === false) {
                $.log("加入店铺会员")
                await getShopOpenCardInfo({ "venderId": 1000001582, "channel": "401" }, 1000001582)
                await bindWithVender({ "venderId": 1000001582, "bindByVerifyCodeFlag": 1, "registerExtend": {}, "writeChildFlag": 0, "activityId": $.openCardActivityId, "channel": 401 }, 1000001582)
                await $.wait(2000)
            } else {
                $.log("已开卡\n")
                await $.wait(2000)
                        ...
                        if (data.result) {
                            switch (function_id) {
                                ...
                                case 'wxActionCommon/getUserInfo':
                                    break;
                                case 'yapei/redenvelope/activityContent':
                                    // console.log(data)
                                    if (!data.data.hasEnd) {
                                        if ($.index === 1) {
                                        ...
                                        $.opencard = data.data.opencard;
                                    } else {
                                        $.log("活动已经结束");
                                    }
                                    break;
                                case 'yapei/redenvelope/checkOpenCard':
