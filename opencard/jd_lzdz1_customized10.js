...
!(async () => {
  ...
  authorCodeList = await getAuthorCodeList('https://raw.githubusercontent.com/okyyds/duck/master/code/abcd.json')
  if ($.getAuthorCodeListerr === false) {
      authorCodeList = [
        'fc89cd2b32e14a14856ab7a6f9b189bc',
      ]
  }
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/april/springUnion/activity/${$.authorNum}?activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}&adsource=null&shareuserid4minipg=null&shopid=undefined&lng=00.000000&lat=00.000000&sid=&un_area=`;
      await member();
      //await $.wait(1000);
      if ($.bean > 0) {
...
async function member() {
  ...
  if ($.token) {
    ...
    if ($.secretPin) {
      ...
      if ($.index === 1) {
        await task("april/springUnion/activityContent", `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=${encodeURIComponent($.authorCode)}`, 0, 1);
      } else {
        await task("april/springUnion/activityContent", `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=${encodeURIComponent($.authorCode)}`);
      }
      $.log("关注店铺");
      await task("april/springUnion/saveTask", `activityId=${$.activityId}&actorUuid=${encodeURIComponent($.actorUuid)}&pin=${encodeURIComponent($.secretPin)}&taskType=23&taskValue=23&shareUuid=${encodeURIComponent($.authorCode)}`);
      //await $.wait(1000);
      await task("taskact/common/drawContent", `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}`);
      //await $.wait(1000);
      await task("april/springUnion/initOpenCard", `pin=${encodeURIComponent($.secretPin)}&activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}`);
      $.log("加入店铺会员");
      if ($.openCardList) {
