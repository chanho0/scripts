...
!(async () => {
  ...
  for (let i = 0; i < helpnum; i++) {
    ...
    $.UserName = decodeURIComponent($.cookie.match(/pt_pin=(.+?);/) && $.cookie.match(/pt_pin=(.+?);/)[1])
    for (let k = 0; k < $.inviteCodeList1.length; k++) {
      if ($.UserName === $.inviteCodeList1[k].use) {
        codeList.push({
          'name': $.UserName,
          'code': $.inviteCodeList1[k].code
        });
      }
    }
...
async function Gethelp() {
  try {
    ...
    $.petidList = [];
    $.crowInfo = {};
    await takeGetRequest('Gethelp');
    
      
      console.log('获取活动信息成功');
      console.log(`互助码：${$.helpInfo.myInfo.shareId}`);
      $.helpCkList.push($.cookie);
      $.inviteCodeList1.push({'use':$.UserName,'code':$.helpInfo.myInfo.shareId,'max':false});
...
async function takeGetRequest(type) {
  ...
  switch (type) {
    ...
    case 'Gethelp':
      url = `https://m.jingxi.com/jxnhj/GetUserInfo?__t=${Date.now() + 2}&dwEnv=7&strInviteId=&nopopup=0&_stk=__t%2CdwEnv%2Cnopopup%2CstrInviteId&_ste=1`;
      ...
      break;
    case 'Dohelp':
      url = `https://m.jingxi.com/jxnhj/DoTask?__t=${Date.now() + 2}&dwEnv=7&taskId=3142&strShareId=${$.oneCodeInfo.code}&bizCode=jxnhj_task&configExtra=&_stk=__t%2CbizCode%2CconfigExtra%2CdwEnv%2CstrShareId%2CtaskId&_ste=1`;
      url += `&h5st=${decrypt(Date.now(), '', '', url)}&&sceneval=2&g_login_type=1&callback=jsonpCBK${String.fromCharCode(Math.floor(Math.random() * 26) + "A".charCodeAt(0))}&g_ty=ls`;
      break;
...
function dealReturn(type, data) {
  switch (type) {
    case 'GetHomePageInfo':
      ...
      if (data.iRet === 0) {
        $.retInfo = data;
        console.log(`助力：${$.retInfo.sErrMsg}\n`);
      } else if(data.iRet === 4001 || data.iRet === 4007|| data.iRet === 30003|| data.iRet === 300012) {
        $.retInfo = data;
        console.log(`助力失败：${$.retInfo.sErrMsg}\n`);
        $.canHelp=false
