        ...
        if (err) {
          ...
          if (data.code === 0) {
              ...
              if (data.data.bizCode === 0) {
                console.log(`膨胀红包:${JSON.stringify(data.data.result.userAwardExpand)}`)
...
function travel_getgrouphelp(){
	//let body={"taskId":taskId,"taskToken":taskToken,"actionType":1,"ss":{"extraData":{"log":"","sceneid":"HYJhPageh5"},"secretp":secretp,"random":randomString(6)}};
  let body={};

	return new Promise((resolve) => {
    console.log("tigernian_pk_getHomeData");
...
function travel_grouphelp(inviteId){
	//let body={"taskId":taskId,"taskToken":taskToken,"actionType":1,"ss":{"extraData":{"log":"","sceneid":"HYJhPageh5"},"secretp":secretp,"random":randomString(6)}};
