...
async function Gethelp() {
  try {
    ...
    await takeGetRequest('Gethelp');
      ...
      console.log('获取活动信息成功');
      console.log(`互助码：${$.helpInfo.token}`);
      $.helpCkList.push($.cookie);
      $.inviteCodeList1.push({'use':$.UserName,'code':$.helpInfo.token,'max':false});
...
async function takeGetRequest(type) {
  ...
  switch (type) {
    ...
    case 'Gethelp':
      ...
      break;
    case 'Dohelp':
      url = `https://m.jingxi.com/pgcenter/sign/helpSign?sceneval=2&token=${$.oneCodeInfo.code}&flag=1&_stk=flag%2Csceneval%2Ctoken&_ste=1`;
      url += `&h5st=${decrypt(Date.now(), '', '', url)}&&sceneval=2&g_login_type=1&callback=jsonpCBK${String.fromCharCode(Math.floor(Math.random() * 26) + "A".charCodeAt(0))}&g_ty=ls`;
      break;
