...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            authorCodeList = [
                'a3f7396d565b4fc688563760f17e0351',
...
async function member() {
    ...
    if ($.token) {
        ...
        if ($.secretPin) {
            ...
            $.log("->game 10次")
            for(let i = 0 ; i<10 ; i++){
                await task('linkgame/addGameScore', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&score=${random(5000, 6000)}`)
                await $.wait(1000)
