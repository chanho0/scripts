/*
10.14~10.20 品牌联合 [gua_UnknownTask6.js]
新增开卡脚本 (脚本已加密
一次性脚本
...
2.关注商品 可能获得10京豆
3.加购商品 可能获得10京豆
  (默认不加购 如需加购请设置环境变量[guaunknownTask_addSku6]为"true"

第一个账号助力作者 其他依次助力CK1
第一个CK失效会退出脚本

每个账号之间延迟 100=延迟100秒 0=延迟0秒会使用每3个账号延迟60秒
guaunknownTaskWait_All 所有
guaunknownTaskWait6="0"


All变量适用
————————————————
入口：[ 10.14~10.20 品牌联合 (https://lzdz-isv.isvjcloud.com/welfare/unite/activity?activityId=1eca438e9a484de0960115fc7088a7c6&shareUuid=77d4cf6ad97242eba39543fea020ece9)]
...
guaunknownTask_addSku = $.isNode() ? (process.env.guaunknownTask_addSku6 ? process.env.guaunknownTask_addSku6 : `${guaunknownTask_addSku}`) : ($.getdata('guaunknownTask_addSku6') ? $.getdata('guaunknownTask_addSku6') : `${guaunknownTask_addSku}`);
...
guaopenwait = $.isNode() ? (process.env.guaunknownTaskWait6 ? process.env.guaunknownTaskWait6 : `${guaopenwait}`) : ($.getdata('guaunknownTaskWait6') ? $.getdata('guaunknownTaskWait6') : `${guaopenwait}`);
guaopenwait = $.isNode() ? (process.env.guaunknownTaskWait_All ? process.env.guaunknownTaskWait_All : `${guaopenwait}`) : ($.getdata('guaunknownTaskWait_All') ? $.getdata('guaunknownTaskWait_All') : `${guaopenwait}`);
...
!(async () => {
  ...
  $.activityId = "1eca438e9a484de0960115fc7088a7c6"
  $.shareUuid = "77d4cf6ad97242eba39543fea020ece9"
  $.venderIds = "1000104855"
  ...
  try {
    ...
    if($.taskData.addSku){
      ...
      if(!$.taskData.addSku.allStatus && !$.outFlag){
        if(guaunknownTask_addSku+"" == "true"){
          flag = true
          $.taskType = 21
          await takePostRequest('加购');
          await $.wait(parseInt(Math.random() * 2000 + 3000, 10))
        }else{
          console.log('如需加购请设置环境变量[guaunknownTaskWait6]为"true"');
        }
      }
    }
