...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            authorCodeList = [
                'ce328b0b63b04758bc6137b8084faebe',
                // '894a9fcd022847ddb041f7e67001b826',
            ]
            $.bean = 0;
            ...
            $.activityId = 'dz2109100000412301'
            $.activityShopId = '1000004123'
            $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/xiaomi/member/activity/${$.authorNum}?activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}&adsource=null&shareuserid4minipg=null&shopid=${$.activityShopId}&lng=00.000000&lat=00.000000&sid=&un_area=`
            await xiaomi();
            await $.wait(5000)
            if ($.bean > 0) {
...
async function xiaomi() {
    ...
    if ($.token) {
        ...
        if ($.secretPin) {
            ...
            await task('xiaomi/member/activityContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=${encodeURIComponent($.pinImg)}&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=${encodeURIComponent($.authorCode)}`)
            await task("taskact/common/drawContent", `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}`)
            // await task('taskact/common/getShareRecord', `activityId=${$.activityId}&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.secretPin)}&num=0&sortStatus=1`)
            if ($.activityContent) {
                $.log("\n-> 任务")
                await $.wait(3000)
                console.log('visitSku')
                for(let i in $.activeTask['visitSku']['data']){
                    console.log($.activeTask['visitSku']['data'][i]['type'])
                    await task('xiaomi/member/saveTask', `activityId=${$.activityId}&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.secretPin)}&taskType=${$.activeTask['visitSku']['data'][i]['type']}&taskValue=${$.activeTask['visitSku']['data'][i]['value']}`)
                    await $.wait(3000)
                }
                await $.wait(3000)
                console.log('addSku')
                await task('xiaomi/member/saveTask', `activityId=${$.activityId}&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.secretPin)}&taskType=21&taskValue=100019791966`)
                await $.wait(3000)
                console.log('followShop')
                for(let i in $.activeTask['followShop']['data']){
                    console.log($.activeTask['followShop']['data'][i]['type'])
                    await task('xiaomi/member/saveTask', `activityId=${$.activityId}&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.secretPin)}&taskType=${$.activeTask['followShop']['data'][i]['type']}&taskValue=${$.activeTask['followShop']['data'][i]['value']}`)
                    await $.wait(3000)
                }
                await $.wait(3000)
                console.log('mainActive')
                for(let i in $.activeTask['mainActive']['data']){
                    console.log($.activeTask['mainActive']['data'][i]['type'])
                    await task('xiaomi/member/saveTask', `activityId=${$.activityId}&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.secretPin)}&taskType=${$.activeTask['mainActive']['data'][i]['type']}&taskValue=${$.activeTask['mainActive']['data'][i]['value']}`)
                    await $.wait(3000)
                }
                // await task('taskact/common/getRankList', `activityId=${$.activityId}&actorUuid=${$.actorUuid}`)
            } else {
                $.log("无法顺利的获取到活动信息。")
            }
        } else {
            ...
            try {
                if (err) {
                    ...
                    if (data) {
                        ...
                        if (data.result) {
                            switch (function_id) {
                                case 'taskact/common/getShareRecord':
                                    // console.log(data)
                                    break;
                                case 'dz/common/getSimpleActInfoVo':
                                    ...
                                    break;
                                case 'xiaomi/member/activityContent':
                                    if (!data.data.hasEnd) {
